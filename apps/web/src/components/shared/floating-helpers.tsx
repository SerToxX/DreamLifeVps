...
import { MessageCircle, X, HelpCircle, ChevronDown, Send } from 'lucide-react';
import { cn } from '@/lib/utils';

// ⚙️  EDITAR AQUÍ
const WHATSAPP_NUMBER = '51999888777';
const WHATSAPP_MENSAJE_DEFAULT = 'Hola Dream Life! Vengo de la web y tengo una consulta';
const HOVER_DELAY_MS = 700; // Cambia a 3000 (3s) o 5000 (5s) si prefieres

// ⚙️  Imagen del botón principal
// Coloca tu archivo en apps/web/public/ y referencia con "/nombre_archivo.png"
// Ejemplos: '/floating_icon.png', '/logo_dreamlife.jpg', '/chat_wsp.png'
const FLOATING_ICON_SRC = '/floating_icon.png';
...
export function FloatingHelpers() {
  ...
  return (
    <>
      ...
      >
        {/* Sub-botón WhatsApp — sale hacia arriba, PEGADO al principal */}
        <a
          href={waUrl}
          target="_blank"
          ...
          aria-label="WhatsApp"
          className={cn(
            'absolute right-1 w-11 h-11 rounded-full bg-[#25D366] hover:bg-[#1ebe5d] text-white shadow-lg flex items-center justify-center transition-all duration-300',
            expanded ? 'bottom-[52px] opacity-100 scale-100 pointer-events-auto' : 'bottom-1 opacity-0 scale-50 pointer-events-none'
          )}
        ...
        {/* Sub-botón FAQ — sale hacia la izquierda, PEGADO al principal */}
        <button
          onClick={() => { setBotOpen(true); setExpanded(false); }}
          aria-label="Preguntas frecuentes"
          className={cn(
            'absolute bottom-1 w-11 h-11 rounded-full bg-card border border-border text-foreground shadow-lg flex items-center justify-center transition-all duration-300 hover:bg-secondary',
            expanded ? 'right-[52px] opacity-100 scale-100 pointer-events-auto' : 'right-1 opacity-0 scale-50 pointer-events-none'
          )}
        ...
        {/* Botón principal — IMAGEN personalizable */}
        <button
          onClick={() => setExpanded(!expanded)}
          aria-label={expanded ? 'Cerrar' : 'Abrir menú de ayuda'}
          className="relative w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center transition-all hover:scale-105 overflow-hidden"
        >
          {expanded ? (
            <X className="w-6 h-6" />
          ) : (
            <img
              src={FLOATING_ICON_SRC}
              alt="Ayuda"
              className="w-full h-full object-cover"
              onError={(e) => {
                // Si la imagen no carga (aún no la subiste), muestra un ícono por defecto
